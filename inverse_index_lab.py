...
def makeInverseIndex(strlist):
    ...
    True
    """
    x = [set(string.split()) for string in strlist] 
    return {word : {index for index in range(0,len(x)) if word in x[index]} for sublist in x for word in sublist}
...
def orSearch(inverseIndex, query):
    ...
    {0, 2, 3, 4, 5}
    """
    return {indexes for query_element in query for indexes in inverseIndex.get(query_element)}
...
def andSearch(inverseIndex, query):
    ...
    {0, 4}
    """
    list_elements = [inverseIndex.get(query_element) for query_element in query]
    first = list_elements[0]

    for index in range(1, len(list_elements)): 
        result = first & list_elements[index]
