...
cartesian_product = [[g,y] for g in ['A','B','C'] for y in [1,2,3]]
...
S = {-4, -2, 1, 2, 5, 0}
# Replace [ ... ] with a one-line list comprehension in which S appears
exclude_zero_list = [(i,j,k) for i in S for j in S for k in S if (i != 0 or j != 0 or k != 0) and sum((i,j,k)) == 0]
...
first_of_tuples_list = [(i,j,k) for i in S for j in S for k in S if (i != 0 or j != 0 or k != 0) and sum((i,j,k)) == 0][0]
				
## 16: (Task 16) List and set differ
# Assign to example_L a list such that len(example_L) != len(list(set(example_L)))
example_L = [0,0,1,2,3,4]
...
range_and_zip = list(zip(range(0,5), ['A','B','C','D','E']))
...
value_list = [last_name[k] for last_name in dlist]
...
value_list_modified_1 = [last_name[k] if k in last_name else "NOT PRESENT" for last_name in dlist] # <-- Use the same expression here
k = 'Frodo'
value_list_modified_2 = [last_name.get(k, "NOT PRESENT") for last_name in dlist] # <-- as you do here
...
D = {'red','white','blue'}
# Replace {...} with a one-line dictionary comprehension
identity_dict = {k:v for (k,v) in [(d,d) for d in D]}
...
listdict2dict = {names[i] : id2salary[i] for i in id2salary.keys()}
...
# Upgraded Task
def all_3_digit_numbers(base, digits): return {x for x in range(0, base**3)}
