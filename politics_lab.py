...
def create_voting_dict(strlist):
    ...
    """
    return {senator_info[0:senator_info.find(' ')] : [int(voting_record) for voting_record in senator_info.split()[3:]] for senator_info in strlist}
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    # assume that both senator's voing record are equal in length
...
def most_similar(sen, voting_dict):
    ...
    """
    most_similar_candidate = sen
    policy_compare_result = 0

    # assume that similarity means that result of dot-product is 0 or greater
    for senator in voting_dict:
        if sen != senator: # exclude senator that we are comparing
            temp_result = policy_compare(sen, senator, voting_dict)
            if temp_result >= policy_compare_result:
                most_similar_candidate = senator
                policy_compare_result = temp_result
...
def least_similar(sen, voting_dict):
    ...
    Example:
        ...
        'b'
    """
    least_similar_candidate = ''
    policy_compare_result = 1000
    ...
    for senator in voting_dict:
        if sen != senator: # exclude senator that we are comparing
            ...
            temp_result = policy_compare(sen, senator, voting_dict)
            if temp_result <= policy_compare_result:
                least_similar_candidate = senator
                policy_compare_result = temp_result

    return least_similar_candidate

## 5: (Task 2.12.5) Chafee, Santorum
# read from file
f = open('US_Senate_voting_data_109.txt')
strlist = [senator_info[:len(senator_info) - 1] for senator_info in f]
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    for senator in sen_set:
        if senator != sen: # exclude senator we are comparing
            dot_product = sum([voting_dict[sen][i] * voting_dict[senator][i] for i in range(len(voting_dict[sen]))])
            dot_product_list.append(dot_product)

    return sum(dot_product_list) / len(dot_product_list)
...
most_average_Democrat = sen_name # give the last name (or code that computes the last name)
