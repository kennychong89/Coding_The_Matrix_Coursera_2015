...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    for item in u.D: # assume that u and v domains are both equal?
        u_value = u.f[item] if item in u.f else 0 # should cover sparsity 
        v_value = v.f[item] if item in v.f else 0 
...
def add(u,v):
    ...
    sum = dict()
    for item in u.D:
        u_value = u.f[item] if item in u.f else 0
        v_value = v.f[item] if item in v.f else 0

        sum[item] = u_value + v_value
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec(v.D, {key: alpha * v[key] for key in v.D if key in v.f})
...
def neg(v):
    ...
    """
    return Vec(v.D, {key: -v[key] for key in v.D if key in v.f})
