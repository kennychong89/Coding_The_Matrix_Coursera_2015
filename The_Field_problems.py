...
def myFilter(L, num):
    ...
    Examples:
      ...
      [15, 25]
    '''
    return [ele for ele in L if ele % num != 0]
...
def my_lists(L):
    ...
    [[], [1, 2, 3]]
    '''
    return [[index for index in range(1,L[curr_elem]+1)] for curr_elem in range(len(L))]
...
def myFunctionComposition(f, g):
    ...
    Examples:
      ...
      True
    '''
    return {f_key: g[f[f_key]] for f_key in f.keys() if f[f_key] in g.keys()}
...
Hint: The value of the Python expression float('infinity') is infinity.
    ...
    -5
    '''
    current = L[0] if len(L) > 0 else float('infinity')
